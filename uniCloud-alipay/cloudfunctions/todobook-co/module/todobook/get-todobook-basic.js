 ...
 * - 内部错误：服务器内部错误
 */
async function getTodoBookBasic(bookId) {
  // 认证验证
  ...
  try {
    ...
    return createSuccessResponse(bookResult.data[0])
  } catch (error) {
    console.error('获取项目册基本信息失败:', error)
    return createErrorResponse(ERROR_CODES.INTERNAL_ERROR, '获取项目册基本信息失败')
  }
}

module.exports = getTodoBookBasic